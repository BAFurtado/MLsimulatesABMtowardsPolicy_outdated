...
def plot_iqrs(data1, title, cols, save = True, format='png'):
...
if __name__ == '__main__':
    ...
    for ACP in acps:
        ...
        plot_iqrs(plot_df, str(ACP), groups_cols.abm_dummies['policies'])
    ...
    for pol in groups_cols.abm_dummies['policies']:
        plot_iqrs(csv.loc[csv[pol] == 1], pol, groups_cols.abm_dummies['acps'])

        # It is now working for policies per ACP, but is too squeezed

    for param in groups_cols.abm_params:

        for ACP in acps:

            if ACP == 'all':
                plot_df = csv.loc[csv[param] == 1]
            else:
                plot_df = csv.loc[csv[param] == 1 & csv[ACP] == 1]

            plot_iqrs(plot_df, str([ACP]+[param]), groups_cols.abm_dummies['policies'])

        for pol in groups_cols.abm_dummies['policies']:
            plot_iqrs(csv.loc[csv[param] == 1 & csv[pol] == 1], str([pol]+[param]), groups_cols.abm_dummies['acps'])
